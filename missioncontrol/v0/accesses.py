...
class Access(object):
    ...
    def _find_boundary(sat, gs, t, step, turn_around_when, stop_when):
        ...
        def _recurse(t, step, alt, azi):
            ...
            turnaround = turn_around_when(sat, gs, alt, alt_next,
                                          azi, azi_next)
            if turnaround:
    ...
    def _find_cutoff(sat, gs, t, step, sigma=0.01):
        ...
        return Access._find_boundary(sat, gs, t, step,
                                     crosses_zero,
                                     alt_is_close_to_cutoff).time
    ...
    def find_max_alt(sat, gs, t, sigma=0.1):
        ...
        return Access._find_boundary(sat, gs, t, 60,
                                     alt_is_lower,
                                     step_is_small).altitude.degrees
    ...
    def to_dict(self):
        return {
            ...
            "_href": urljoin(
                self._base_url,
                "v0/accesses/{access_id}/".format(
                    access_id=self.access_id
                )
            ),
            "_track": urljoin(
                self._base_url,
                "v0/accesses/{access_id}/track/".format(
                    access_id=self.access_id
                )
            )
        }
    ...
    def __repr__(self):
        return ("<Access: ({satellite}, {groundstation})"
                " ({start_time}, {end_time})"
                " ({max_alt} deg)>").format(**self.to_dict())
...
class AccessCalculator(object):
    ...
    @classmethod
    def calculate_accesses(cls, satellites, groundstations, start_time=None,
                           end_time=None, filter_func=None):
        ...
        with ProcessPoolExecutor() as executor:
            for sat, gs, access_times in executor.map(_find_accesses_wrapper,
                                                      pairs):
                accesses += [
...
def search(limit=100, range_start=None, range_end=None, range_inclusive='both',
           satellites=None, groundstations=None):
    ...
    return [access.to_dict() for access in
            sorted(accesses, key=lambda a: a.start_time.tt)][:limit]
...
class CachedAccessCalculator(AccessCalculator):
    ...
    def _cached_pair_compute(cls, sat, gs, tbucket):
        ...
        tle1, tle2 = sat.tle
        bucket_hash = cls._sat_gs_vector_hash(
            tle1, tle2,
            gs.latitude, gs.longitude, gs.elevation, gs.horizon_mask
        ) + '@' + str(start) + "-" + str(end)

        cached = CachedAccess.objects.filter(bucket_hash=bucket_hash).exists()
        if not cached:
            # compute accesses
            sat, gs, found = _find_accesses(sat, gs, start, end,
                                            cls.timescale)
            accesses_to_cache = [
                Access(rising, setting, sat, gs, max_alt)
                for rising, setting, max_alt in found if start.tai <= rising.tai < end.tai
            ]
    ...
    @classmethod
    def calculate_accesses(cls, satellites, groundstations, start_time=None,
                           end_time=None, filter_func=None, limit=100):
        """ calculates all of the access between a given list of satellites
            and groundstations over the range between start_time and end_time.
        ...
        accesses = cls._chunked_compute(satellites, groundstations,
