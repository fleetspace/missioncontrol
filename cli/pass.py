...
def _make_rows(data, headers, defaults={}):
    ...
    table_data = []
    for o in data:
        table_data.append([o.get(k, defaults.get(k)) for k in headers])
    return table_data
...
def list_all(args):
    ...
    headers = ["t", "satellite", "groundstation", "start_time", "end_time", "max_alt"]
    table_data = []
    table_data += _make_rows(accesses, headers + ["id"], {"t": "a"})
    table_data += _make_rows(passes, headers + ["uuid"], {"t": "p"})
    if table_data:
        table_data = sorted(table_data, key=lambda w: w[headers.index("start_time")])
...
def create_pass_from_access(args):
    new_pass = args.mc_api.put_pass(args.pass_id, access_id=args.access_id)
...
def cancel_pass(args):
    _pass = args.mc_api.patch_pass(args.pass_id, is_desired=False)
    print(json.dumps(_pass, indent=4))


def uncancel_pass(args):
    _pass = args.mc_api.patch_pass(args.pass_id, is_desired=True)
    print(json.dumps(_pass, indent=4))
...
def main():
    ...
    list_parser.add_argument("--satellites", help="csv satellite hwids")
    list_parser.add_argument("--groundstations", help="csv groundstation hwids")
    list_parser.add_argument(
        "--all", action="store_true", help="also shows stale passes"
    )

    list_passes_parser = subparsers.add_parser("list_passes")
    list_passes_parser.set_defaults(func=list_passes)
    list_passes_parser.add_argument("--satellites", help="csv satellite hwids")
    list_passes_parser.add_argument("--groundstations", help="csv groundstation hwids")
    list_passes_parser.add_argument(
        "--all", action="store_true", help="also shows stale passes"
    )

    list_accesses_parser = subparsers.add_parser("list_accesses")
    list_accesses_parser.set_defaults(func=list_accesses)
    list_accesses_parser.add_argument("--satellites", help="csv satellite hwids")
    list_accesses_parser.add_argument(
        "--groundstations", help="csv groundstation hwids"
    )
    list_accesses_parser.add_argument(
        "--range_start",
    ...
    create_parser.add_argument("--pass-id", default=uuid.uuid4(), type=uuid.UUID)
    ...
    tcreate_parser.add_argument("--pass-id", default=uuid.uuid4(), type=uuid.UUID)
    ...
    set_parser.add_argument("val", type=bool)
